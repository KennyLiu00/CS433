...
void in_redirect(char *file) // < , sort < in.txt , file will serve as input to the sort
{
...
void out_redirect(char *file) 
{
  int o = open(file, O_WRONLY | O_APPEND | O_TRUNC | O_CREAT, 0600);
  dup2(o, 1);
  close(o);
   ...
   while (tok != NULL) // going through the command
     {
      if(*tok == '<')//checking if the character is an input
        in_redirect(strtok(NULL, " "));//call the input redirect function
      else if(*tok == '>')//checking if the character is an output
        out_redirect(strtok(NULL, " "));//call the output redirection function
...
void execArgs(char command[], char *args[]) 
{
  ...
  /* used when a pipe is not in the command */
  if (pid < 0) // error 
  {
    printf("Fork Failed");
  ...
  while (should_run) //when the program is not determined to be exited
  {
    ...
    else if(strcmp(command, "!!") == 0) // displays last command, executes it
    {
      ...
      else 
      {
        ...
        execArgs(command, args); //stores the args to be forked
      }
    }
    else // normal command
    { 
      ...
      // if &, <, > is found in the command, and the next index has no space
      // add space 
